...
interface InBlogImagePromptProps {
  content: string;
}
...
const InBlogImagePrompt: React.FC<InBlogImagePromptProps> = ({
  ...
  // Fetch system prompt from file and transform content
  useEffect(() => {
    const fetchSystemPrompt = async () => {
      try {
        ...
        if (data.prompt) {
          ...
          // Combine system prompt with transformed blog content
          setFullPrompt(`${data.prompt}\n\n${transformedContent}`);
        }
      } catch (err) {
        console.error('Failed to load in-blog image system prompt:', err);
        // Use transformed content only if system prompt fails to load
        setFullPrompt(transformContent(content));
      }
    };
  ...
  return (
    <div className="p-6 dark:bg-gray-800">
      ...
      <div className="mt-4 text-sm text-gray-500 dark:text-gray-300 flex items-center p-2 bg-gray-50 dark:bg-gray-800 border dark:border-gray-700 rounded-md shadow-sm">
        ...
        <p>
          <span className="font-medium">Note:</span> This combined prompt can be used with AI image generation tools to create in-blog images for your post.
        </p>
      </div>
    </div>
  );
...
export default InBlogImagePrompt;
